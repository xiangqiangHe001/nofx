...
func (o *OKXTrader) SetLeverage(symbol string, leverage int) error {
	...
	if err := json.Unmarshal(respBody, &resp); err != nil {
		return fmt.Errorf("解析设置杠杆响应失败: %w", err)
	}
	if resp.Code != "0" {
		...
		if resp.Code == "51000" && strings.Contains(strings.ToLower(resp.Msg), "posside") {
			for _, side := range []string{"long", "short"} {
				...
				if err2 != nil {
					return fmt.Errorf("设置杠杆失败(%s): %w", side, err2)
				}
				var resp2 struct {
					Code string `json:"code"`
					Msg  string `json:"msg"`
				...
				if resp2.Code != "0" {
					return fmt.Errorf("设置杠杆失败(%s): code=%s msg=%s", side, resp2.Code, resp2.Msg)
				}
			}
			return nil
		}
...
func (o *OKXTrader) doSignedRequest(method, path, body string) ([]byte, error) {
    ...
    fb := "http://127.0.0.1:7897"
    tr := &http.Transport{Proxy: func(_ *http.Request) (*url.URL, error) { return url.Parse(fb) }}
...
func (o *OKXTrader) getCTVal(instID string) float64 {
	...
	if lot > 0 {
		o.lotSzCache[instID] = lot
	}
	if min > 0 {
		o.minSzCache[instID] = min
	}
	o.cacheMu.Unlock()
	return ct
}
